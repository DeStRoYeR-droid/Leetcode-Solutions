...
**Difficulty:** Hard  
[View on LeetCode](https://leetcode.com/problems/minimum-inversion-count-in-subarrays-of-fixed-length/)
...
There is only one subarray of length `k = 4`: `[5, 3, 2, 1]`. **Within this subarray, the inversions are: `(0, 1)`, `(0, 2)`, `(0, 3)`, `(1, 2)`, `(1, 3)`, and `(2, 3)`.  
...
All subarrays of length `k = 1` contain only one element, so no inversions are possible. **The minimum inversion count is therefore 0.
